...
test('should send a coffee message if not present user tag', async () => {
  ...
  const message = new Discord.Message(
    client,
    {
      ...
      content: `${process.env.PREFIX}caffè`,
      author: js.user,
    },
  ...
  const messages = getGeneralChannelMessages()
  expect(messages).toHaveLength(1)
  expect(messages[0].content).toEqual(`Ciao <@${js.user.id}> eccoti un buon :coffee:`)
})
...
test('should send a coffee message if is present user tag', async () => {
  ...
  expect(messages[0].content).toEqual(
    `Ciao <@${mentionedUsers[0].id}> sei fortunato, l'utente <@${js.user.id}> ti ha offerto un :coffee:`,
  )
})
