...
module.exports = class Caffe extends Commands {
	constructor(client) {
		...
		this.alias = 'ca';
		this.args =
      '[@nomeutente (Opzionale)] Se vuoi offrire il caffè ad un\'altra persona';
		this.example = `${client.conf.prefix}caffè @nomeutente`;
		this.description =
      'Ideala per bere un caffè in compagnia o offerto dal bot 👊🏻';
		this.timer = 0;
	...
	async execution(message, bot) {
		...
		if (message.mentions.members.first()) {
			...
			message.channel
				.send(`Ciao <@${id_author}> eccoti un buon :coffee:`)
				.catch((e) => {
					console.log(e);
				});
		}
