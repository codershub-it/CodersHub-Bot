...
module.exports = class Hint extends Commands {
	...
	async execution(message, bot) {
		...
		const hintChannel = this.client.channels.cache.find(channel => channel.name === 'suggerimenti');
        ...
        if (hintMessage.length < 5) {
            ...
            emb.setTitle(`Nuovo suggerimento da ${(message.author.username)? message.author.username : message.author.name }`);
            ...
            message.reply(' la proposta è stato inviata. Grazie per aver contribuito a migliorare la Community!');
