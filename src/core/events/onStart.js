...
async function printStructureServer(client, channel_id) {
  ...
  emb.setColor('RANDOM')
  await channel.send(emb).catch((err) => {
    console.log(err)
  })
  ...
  for (const e of _server) {
    ...
    const emb_sub = new client._botMessageEmbed()
    emb_sub.setTitle(`${e.name} canali sono`)
    emb_sub.setDescription(sub_channel)
    await channel.send(emb_sub).catch((err) => {
      console.log(err)
    })
  }
}
