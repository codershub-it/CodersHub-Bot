...
module.exports = class TestList extends Commands {
    ...
    async execution(message, bot) {
        let currentPage = 0
        const embeds = this.generateQueueEmbed(this.exemplce_element)
        ...
        collector.on('collect', async (reaction, user) => {
            if (reaction.emoji.name === '➡️') {
                if (currentPage < embeds.length -1 ) {
                    currentPage++
                    await queueEmbed.edit(`Current Page: ${currentPage + 1}/${embeds.length}`, embeds[currentPage])
    ...
    generateQueueEmbed(queue){
        ...
        for (let i = 0; i < queue.length; i += 10) {
            ...
            let j = i
            k += 10
            const info = current.map(track => `${++j} - "this supports [named links](https://discordapp.com) on top of the previously shown subset of markdown.  [${track.name}][${track.url}]`).join('\n')
            const embed = new client._botMessageEmbed()
                .setDescription(`**[current info: ${queue[0].name}]**\n${info}`)
            embeds.push(embed)
        }
        return embeds
    }
