	...
	async execution(message, bot) {
		...
		if (!nomeComando) {
			...
			Object.entries(commands).filter(([, fn]) => {
				if (fn.displayHelp === 1) {
					if (
						message.member.roles.cache.some((itm) =>
							fn.access.includes(itm.name),
						)
					) {
						if (fn.cmd !== 'help') {
		...
		Object.entries(commands).filter(([, fn]) => {
			if (
				fn.displayHelp == 1 &&
        (fn.cmd == nomeComando || fn.alias == nomeComando)
			) {
				if (
					message.member.roles.cache.some((itm) => fn.access.includes(itm.name))
				) {
