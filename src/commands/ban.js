...
module.exports = class Ban extends Commands {
	...
	async execution(message, bot) {
         ...
         */
		if (member && day_ban) {
			member.ban({ days: day_ban, reason: reason }).then((GuildMember) => {
				message.reply(`:wave: ${message.displayName} hai effettuato il ban con successo! https://gfycat.com/playfulfittingcaribou :point_right:`);
			}).catch(() => {
