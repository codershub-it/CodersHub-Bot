function init(client) {
  client.on('guildMemberAdd', async (member) => {
    try {
      ...
      await sendPmToNewMember(member)
    } catch (e) {
...
async function sendPmToNewMember(member) {
  await member.user.send('Ciao!', {
    embed: {
      title: 'Benvenuto su CodersHub!',
      description:
        "Se vuoi scegliere quali sezioni vedere e che tipo di notifiche ricevere all'interno del nostro server, puoi andare nel canale <#805724080111353916> e personalizzare le impostazioni",
      color: 'RANDOM',
      thumbnail: {
